...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match integrated_system.process_user_command("generate".to_string(), vec!["Create a simple function to add two numbers in Rust".to_string()]).await {
        Ok(output) => {
    ...
    let _ = integrated_system.shell_adapter.execute_shell_command(
        format!("echo '{}' > temp_test.rs", temp_code)
    ).await;
    
    match integrated_system.process_user_command("analyze".to_string(), vec!["temp_test.rs".to_string()]).await {
        Ok(output) => {
    ...
    println!("\n🧹 Cleanup");
    println!("---------");
    let _ = integrated_system.shell_adapter.execute_shell_command("rm -f temp_test.rs".to_string()).await;
