...
pub async fn run(runner: &mut CliRunner, args: GenerateArgs) -> Result<(), Box<dyn std::error::Error>> {
    runner.print_info(&format!("🤖 Generating code from prompt: \"{}\"", args.prompt));
    ...
    if runner.verbose() {
        runner.print_verbose(&format!("Generation request: language={:?}, strategy={}", 
...
async fn build_generation_request(runner: &mut CliRunner, args: &GenerateArgs) -> Result<GenerationRequest, Box<dyn std::error::Error>> {
    ...
    let mut request = GenerationRequest {
        ...
        file_path: args.output.as_ref().map(|p| p.to_string_lossy().to_string()),
        context,
        config,
        constraints: Vec::new(),
    };
...
fn display_generated_code(runner: &CliRunner, code: &str, args: &GenerateArgs) -> Result<(), Box<dyn std::error::Error>> {
...
async fn save_generated_code(runner: &CliRunner, code: &str, args: &GenerateArgs) -> Result<(), Box<dyn std::error::Error>> {
    let output_path = if let Some(path) = &args.output {
        path.clone()
    } else {
        // Generate a filename based on the prompt and language
        let sanitized_prompt = args.prompt
            .chars()
            .filter(|c| c.is_alphanumeric() || *c == '_' || *c == '-')
            .take(30)
            .collect::<String>();
            
        let extension = args.language.as_deref().map(|lang| match lang {
            "rust" => "rs",
            "python" => "py", 
            "javascript" => "js",
            "typescript" => "ts",
        ...
        PathBuf::from(format!("generated_{}_{}.{}", 
    ...
    let chars = code.len();
    runner.print_info(&format!("📊 Generated {} lines ({} characters)", lines, chars));
