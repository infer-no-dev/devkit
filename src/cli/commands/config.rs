...
pub async fn run(
    ...
    match command {
        ConfigCommands::Show { path } => show_config(runner, path.as_deref()).await,
        ConfigCommands::Get { path } => get_config_value(runner, &path).await,
        ConfigCommands::Set { path, value } => set_config_value(runner, &path, &value).await,
        ConfigCommands::Validate => validate_config(runner).await,
        ConfigCommands::Environment { env } => switch_environment(runner, &env).await,
        ConfigCommands::Environments => list_environments(runner).await,
        ConfigCommands::Edit => edit_config(runner).await,
        ConfigCommands::Reset { section } => reset_config(runner, section.as_deref()).await,
        ConfigCommands::Export { output, format } => export_config(runner, &output, &format).await,
        ConfigCommands::Import { input, merge } => import_config(runner, &input, merge).await,
    }
}
...
fn print_config_text(runner: &CliRunner, config: &crate::config::Config) {
    ...
    println!(
        "  • Default Priority: {}",
        config.agents.default_agent_priority
    );

    println!("\n🎨 Code Generation:");
    println!(
        "  • Indentation: {}",
        config.codegen.default_style.indentation
    );
    println!(
        "  • Indent Size: {}",
        config.codegen.default_style.indent_size
    );
    println!(
        "  • Line Length: {}",
        config.codegen.default_style.line_length
    );
    println!(
        "  • Naming Convention: {}",
        config.codegen.default_style.naming_convention
    );

    println!("\n🧠 AI Model Settings:");
    println!(
        "  • Default Provider: {}",
        config.codegen.ai_model_settings.default_provider
    );
    println!(
        "  • Default Model: {}",
    ...
    println!(
        "  • Temperature: {}",
        config.codegen.ai_model_settings.temperature
    );
    println!(
        "  • Max Tokens: {}",
        config.codegen.ai_model_settings.max_tokens
    );
    println!(
        "  • Context Window: {}",
        config.codegen.ai_model_settings.context_window_size
    );

    println!("\n🔗 Ollama Settings:");
    println!(
        "  • Endpoint: {}",
        config.codegen.ai_model_settings.ollama.endpoint
    );
    println!(
        "  • Timeout: {}s",
        config.codegen.ai_model_settings.ollama.timeout_seconds
    );
    println!(
        "  • Max Retries: {}",
        config.codegen.ai_model_settings.ollama.max_retries
    );
...
fn print_config_table(runner: &CliRunner, config: &crate::config::Config) {
    ...
    println!(
        "│ {:<31} │ {:<34} │",
        "AI Provider", config.codegen.ai_model_settings.default_provider
    );
    println!(
        "│ {:<31} │ {:<34} │",
        "AI Model", config.codegen.ai_model_settings.default_model
    );
    println!(
        "│ {:<31} │ {:<34} │",
        "Ollama Endpoint", config.codegen.ai_model_settings.ollama.endpoint
    );
    println!(
        "│ {:<31} │ {:<34} │",
        "Temperature", config.codegen.ai_model_settings.temperature
    );
    println!(
        "│ {:<31} │ {:<34} │",
        "Max Tokens", config.codegen.ai_model_settings.max_tokens
    );
    println!(
        "│ {:<31} │ {:<34} │",
        "Max Concurrent Agents", config.agents.max_concurrent_agents
    );
    println!(
        "│ {:<31} │ {:<34} │",
        "Agent Timeout (s)", config.agents.agent_timeout_seconds
    );
