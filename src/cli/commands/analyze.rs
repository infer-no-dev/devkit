use crate::cli::{CliRunner, AnalyzeArgs, OutputFormat};
use crate::context::AnalysisConfig;
use indicatif::{ProgressBar, ProgressStyle};
use std::time::Instant;
use std::path::PathBuf;

pub async fn run(runner: &mut CliRunner, args: AnalyzeArgs) -> Result<(), Box<dyn std::error::Error>> {
    ...
    runner.print_info(&format!("🔍 Starting codebase analysis of {} target{}", 
        targets.len(), 
    ...
    if args.include_tests {
        ...
        analysis_config.exclude_patterns.retain(|pattern| {
    ...
    if !args.analysis_types.is_empty() && verbose {
        runner.print_verbose(&format!("Specific analysis types requested: {:?}", args.analysis_types));
    ...
    for (i, target) in targets.iter().enumerate() {
        if let Some(ref pb) = progress_bar {
            pb.set_message(format!("Analyzing target {}/{}: {}", 
                i + 1, targets.len(), target.display()));
        }
        ...
        let result = {
            ...
            context_manager.analyze_codebase(target.clone(), analysis_config.clone()).await
        };
    ...
    if let Some(export_path) = args.export {
        runner.print_info(&format!("📄 Exporting results to: {}", export_path.display()));
    ...
    runner.print_success(&format!("🎉 Analysis completed in {:.2}s", duration.as_secs_f64()));
...
fn format_text_output(contexts: &[crate::context::CodebaseContext]) -> String {
    ...
    for (i, context) in contexts.iter().enumerate() {
        ...
        output.push_str(&format!("⏱️  Analysis Duration: {}ms\n", context.metadata.analysis_duration_ms));
        ...
        if !context.dependencies.is_empty() {
            ...
            for dep in context.dependencies.iter().take(10) { // Show first 10
                let version = dep.version.as_deref().unwrap_or("*");
                output.push_str(&format!("  • {} ({}): {}\n", dep.name, version, format!("{:?}", dep.dependency_type)));
            }
        ...
        if let Some(ref repo_info) = context.repository_info {
            output.push_str(&format!("\n🌿 Repository: {} (Git)", repo_info.root_path.display()));
            ...
            output.push_str(&format!("  • Clean: {}\n", if repo_info.status.is_clean { "Yes" } else { "No" }));
        }
        ...
        if let Some(ref semantic) = context.semantic_analysis {
            ...
            output.push_str(&format!("  • Context Suggestions: {}\n", semantic.context_suggestions.len()));
        }
...
fn format_json_output(contexts: &[crate::context::CodebaseContext]) -> Result<String, Box<dyn std::error::Error>> {
    Ok(serde_json::to_string_pretty(contexts)?)
}

fn format_yaml_output(contexts: &[crate::context::CodebaseContext]) -> Result<String, Box<dyn std::error::Error>> {
...
fn format_table_output(contexts: &[crate::context::CodebaseContext]) -> String {
    ...
    for context in contexts {
        let path = context.root_path.file_name()
            .and_then(|n| n.to_str())
            .unwrap_or(".")
            .chars()
            .take(19)
