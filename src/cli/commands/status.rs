...
use crate::cli::{CliRunner, StatusArgs, OutputFormat};
use super::utils::*;
...
pub async fn run(runner: &mut CliRunner, args: StatusArgs) -> Result<(), Box<dyn std::error::Error>> {
...
async fn get_system_status(runner: &mut CliRunner, args: &StatusArgs) -> Result<SystemStatus, Box<dyn std::error::Error>> {
...
async fn check_configuration_status(runner: &mut CliRunner) -> Result<ComponentStatus, Box<dyn std::error::Error>> {
    let start_time = Instant::now();
    ...
    let (status, message) = match runner.config_manager.validate() {
        Ok(_) => {
            ...
            details.insert("config_file".to_string(), config_path.to_string_lossy().to_string());
            details.insert("file_exists".to_string(), config_path.exists().to_string());
            
            // Check environment
            details.insert("environment".to_string(), runner.config_manager.environment().to_string());
            
            (StatusLevel::Healthy, "Configuration is valid and loaded".to_string())
        }
        Err(e) => {
            details.insert("validation_error".to_string(), e.to_string());
            (StatusLevel::Error, format!("Configuration validation failed: {}", e))
        }
    };
...
async fn check_context_status(runner: &mut CliRunner) -> Result<ComponentStatus, Box<dyn std::error::Error>> {
    let start_time = Instant::now();
    let mut details = HashMap::new();
    ...
    let (status, message) = if runner.context_manager.is_some() {
        ...
        details.insert("working_directory".to_string(), current_dir.to_string_lossy().to_string());
        ...
        if is_project {
            ...
            (StatusLevel::Warning, "Not in a recognized project directory".to_string())
        }
    } else {
        details.insert("initialized".to_string(), "false".to_string());
        (StatusLevel::Warning, "Context system not initialized".to_string())
    };
...
async fn check_agent_status(runner: &mut CliRunner) -> Result<ComponentStatus, Box<dyn std::error::Error>> {
    let start_time = Instant::now();
    let mut details = HashMap::new();
    ...
    let (status, message) = if runner.agent_system.is_some() {
        ...
        if let Some(agent_system) = &runner.agent_system {
            ...
            let idle_count = agent_statuses.iter()
                .filter(|(_, status)| matches!(status, crate::agents::AgentStatus::Idle))
                .count();
            details.insert("idle_agents".to_string(), idle_count.to_string());
            
            (StatusLevel::Healthy, format!("Agent system running with {} agents", agent_statuses.len()))
        } else {
            (StatusLevel::Warning, "Agent system reference not available".to_string())
        }
    } else {
        details.insert("initialized".to_string(), "false".to_string());
        (StatusLevel::Warning, "Agent system not initialized".to_string())
    };
...
async fn check_shell_status() -> Result<ComponentStatus, Box<dyn std::error::Error>> {
    ...
    details.insert("completions_installed".to_string(), completions_installed.to_string());
    
    let (status, message) = if is_supported {
        if completions_installed {
            (StatusLevel::Healthy, format!("Shell integration active ({})", shell_name))
        } else {
            (StatusLevel::Warning, format!("Shell integration available but not installed ({})", shell_name))
        }
    } else {
        (StatusLevel::Warning, format!("Shell not fully supported ({})", shell_name))
    };
...
async fn check_external_dependencies() -> Result<Vec<ComponentStatus>, Box<dyn std::error::Error>> {
    ...
    components.push(ComponentStatus {
        name: "Git".to_string(),
        status: if git_status.0 { StatusLevel::Healthy } else { StatusLevel::Warning },
    ...
    components.push(ComponentStatus {
        ...
        status: if node_status.0 { StatusLevel::Healthy } else { StatusLevel::Warning },
    ...
    components.push(ComponentStatus {
        ...
        status: if python_status.0 { StatusLevel::Healthy } else { StatusLevel::Warning },
        ...
        .output() 
    {
        Ok(output) => {
            ...
            if output.status.success() {
                (true, String::from_utf8_lossy(&output.stdout).to_string(), duration)
            } else {
                (false, String::from_utf8_lossy(&output.stderr).to_string(), duration)
            }
        }
        Err(_) => {
            let duration = start_time.elapsed();
...
async fn get_performance_metrics(runner: &mut CliRunner) -> Result<PerformanceMetrics, Box<dyn std::error::Error>> {
...
fn display_text_status(runner: &CliRunner, status: &SystemStatus, detailed: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    runner.print_output(&format!("{} Overall Status: {}\n", overall_emoji, overall_text), None);
    
    if let Ok(_duration) = status.timestamp.duration_since(std::time::UNIX_EPOCH) {
        runner.print_output(&format!("🕐 Last Check: {}\n\n", 
            chrono::DateTime::<chrono::Utc>::from(status.timestamp).format("%Y-%m-%d %H:%M:%S UTC")), None);
    }
    ...
    if let Some(metrics) = &status.performance_metrics {
        runner.print_output("\n📊 Performance Metrics:\n", None);
        runner.print_output("────────────────────────\n", None);
        runner.print_output(&format!("💾 Memory Usage: {}\n", format_file_size(metrics.memory_usage)), None);
        runner.print_output(&format!("🔥 CPU Usage: {:.1}%\n", metrics.cpu_usage), None);
        runner.print_output(&format!("💿 Disk Usage: {}\n", format_file_size(metrics.disk_usage)), None);
        runner.print_output(&format!("🤖 Active Agents: {}\n", metrics.active_agents), None);
        runner.print_output(&format!("📦 Context Cache: {} items\n", metrics.context_cache_size), None);
        runner.print_output(&format!("⚡ Avg Response Time: {:?}\n", metrics.avg_response_time), None);
    }
    ...
    if detailed {
        runner.print_output("\n💻 System Information:\n", None);
        runner.print_output("─────────────────────\n", None);
        runner.print_output(&format!("OS: {} ({})\n", status.system_info.os, status.system_info.arch), None);
        runner.print_output(&format!("Rust Version: {}\n", status.system_info.rust_version), None);
        runner.print_output(&format!("Agentic Version: {}\n", status.system_info.agentic_version), None);
        runner.print_output(&format!("Working Directory: {}\n", status.system_info.working_directory.display()), None);
...
fn display_table_status(runner: &CliRunner, status: &SystemStatus, detailed: bool) -> Result<(), Box<dyn std::error::Error>> {
