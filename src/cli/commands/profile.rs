...
pub async fn run(
    ...
    match command {
        ...
        ProfileCommands::Diagnostics => show_system_diagnostics(runner).await?,
        ProfileCommands::Memory { detailed } => analyze_memory_usage(runner, detailed).await?,
    }
...
async fn profile_system_performance(
    ...
    runner.print_info(&format!(
        "Profiling system performance for {} seconds...",
        duration
    ));
    ...
    let avg_memory_percent =
        memory_samples.iter().map(|(_, p)| p).sum::<f64>() / memory_samples.len() as f64;
    ...
    match runner.format() {
        ...
        _ => {
            runner.print_success(&format!(
                "System profiling completed in {:.2}s",
                elapsed.as_secs_f64()
            ));
            println!();
...
async fn profile_agent_performance(
    ...
    runner.print_info(&format!(
        "Profiling {} performance for {} seconds...",
        agent_name, duration
    ));
    ...
    match runner.format() {
        ...
        _ => {
            runner.print_success(&format!(
                "Agent profiling completed in {:.2}s",
                elapsed.as_secs_f64()
            ));
            println!();
            ...
            runner.print_info(
                "Note: Full agent profiling requires completed agent system implementation",
            );
        }
    }
...
async fn profile_context_analysis(
    ...
    runner.print_info(&format!(
        "Profiling context analysis for: {}",
        target.display()
    ));
    ...
    if let Some(context_manager) = runner.context_manager_mut() {
        let analysis_result = context_manager
            .analyze_directory(&target, include_breakdown)
            .await;
        let elapsed = start_time.elapsed();

        match analysis_result {
            Ok(context) => match runner.format() {
                crate::cli::OutputFormat::Json => {
                ...
                _ => {
                    runner.print_success(&format!(
                        "Context analysis completed in {:.2}s",
                        elapsed.as_secs_f64()
                    ));
                    println!();
                    ...
                    println!(
                        "   Total Size: {:.2} MB",
                        context.metadata.total_size_bytes as f64 / 1024.0 / 1024.0
                    );
                    println!(
                        "   Processing Rate: {:.1} files/sec",
                        context.files.len() as f64 / elapsed.as_secs_f64()
                    );
...
async fn show_system_diagnostics(runner: &CliRunner) -> Result<(), Box<dyn std::error::Error>> {
...
async fn analyze_memory_usage(
    ...
    let devkit_processes: Vec<_> = system
        .processes()
        .values()
        .filter(|p| {
    ...
    match runner.format() {
        ...
        _ => {
            ...
            if detailed && !devkit_processes.is_empty() {
                ...
                println!();
                println!(
                    "   Total DevKit Memory: {:.2} MB",
                    total_memory as f64 / 1024.0 / 1024.0
                );
                println!(
                    "   % of System Memory: {:.3}%",
                    (total_memory as f64 / system.total_memory() as f64) * 100.0
                );
            }
