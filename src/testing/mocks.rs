...
impl Agent for MockAgent {
    ...
    fn get_metrics(&self) -> crate::agents::AgentMetrics {
        crate::agents::AgentMetrics::default()
    }

    async fn shutdown(&mut self) -> Result<(), AgentError> {
        *self.status.lock().unwrap() = AgentStatus::Offline;
        Ok(())
    }

    async fn process_task(&mut self, task: AgentTask) -> Result<AgentResult, AgentError> {
        // Update status
        *self.status.lock().unwrap() = AgentStatus::Processing {
        ...
        if self.should_fail {
            *self.status.lock().unwrap() = AgentStatus::Error {
            ...
            return Err(AgentError::TaskExecutionFailed(
                "Mock processing failed".to_string(),
            ));
        }
        ...
        *self.status.lock().unwrap() = AgentStatus::Idle;
        ...
        Ok(AgentResult {
            ...
            processing_duration_ms: self.processing_delay.as_millis() as u64,
...
impl MockCodeGenerator {
    fn generate_code(
        ...
        if self.should_fail {
            return Err(crate::codegen::CodeGenError::GenerationFailed(
...
impl MockCodebaseAnalyzer {
    fn analyze_file(
        ...
        _config: &crate::context::AnalysisConfig,
    ) -> Result<crate::context::FileContext, crate::context::ContextError> {
        if self.should_fail {
            return Err(crate::context::ContextError::AnalysisFailed(
                "Mock analysis failed".to_string(),
            ));
        }
    ...
    fn analyze_codebase(
        &mut self,
        _files: &[crate::context::FileContext],
    ) -> Result<crate::context::CodebaseContext, crate::context::ContextError> {
        if self.should_fail {
            return Err(crate::context::ContextError::AnalysisFailed(
                "Mock codebase analysis failed".to_string(),
            ));
        }
...
impl MockCommandExecutor {
    fn execute_command(
        ...
        command: &str,
        _args: &[&str],
    ) -> Result<String, crate::shell::ShellError> {
        if self.should_fail {
            return Err(crate::shell::ShellError::ExecutionFailed(
                "Mock command failed".to_string(),
            ));
        }

        let full_command = format!("{} {}", command, _args.join(" "))
            .trim()
            .to_string();
        self.executed_commands
            .lock()
            .unwrap()
            .push(full_command.clone());

        if let Some((output, exit_code)) = self.command_results.get(&full_command) {
            if *exit_code == 0 {
                Ok(output.clone())
            } else {
                Err(crate::shell::ShellError::ExecutionFailed(format!(
    ...
    fn execute_command_async(
        ...
        command: &str,
        args: &[&str],
    ) -> Result<tokio::process::Child, crate::shell::ShellError> {
        ...
        if self.should_fail {
            return Err(crate::shell::ShellError::ExecutionFailed(
                "Mock async command failed".to_string(),
            ));
        }

        // This is a mock implementation - in real tests we might use a different approach
        Err(crate::shell::ShellError::ExecutionFailed(
            "Mock async not implemented".to_string(),
        ))
    }
...
mod tests {
    ...
    async fn test_mock_agent_failure() {
        ...
        assert!(matches!(status, AgentStatus::Error { message: _ }));
    }
    ...
    fn test_mock_language_detector() {
        ...
        assert_eq!(
            detector.detect_language("fn main() {}"),
            Some("rust".to_string())
        );
        assert_eq!(
            detector.detect_language("def main():"),
            Some("python".to_string())
        );
        assert_eq!(
            detector.detect_language("function main() {}"),
