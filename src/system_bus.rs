...
pub enum SystemEvent {
    // UI Events
    UIUserInput { input: String, context: serde_json::Value },
    UICommandRequest { command: String, args: Vec<String> },
    UIProgressUpdate { component: String, progress: f32, message: String },
    UINotification { level: NotificationLevel, title: String, message: String },
    
    // Agent Events
    AgentTaskStarted { agent_id: String, task_id: String, description: String },
    AgentTaskCompleted { agent_id: String, task_id: String, result: serde_json::Value },
    AgentTaskFailed { agent_id: String, task_id: String, error: String },
    AgentStatusChanged { agent_id: String, status: String },
    
    // Code Generation Events
    CodeGenerationRequested { request_id: String, prompt: String, language: Option<String> },
    CodeGenerationCompleted { request_id: String, code: String, metadata: serde_json::Value },
    CodeGenerationFailed { request_id: String, error: String },
    
    // AI Events
    AIModelLoaded { provider: String, model: String },
    AIRequestStarted { request_id: String, prompt: String },
    AIResponseReceived { request_id: String, response: String, tokens_used: usize },
    AIError { request_id: String, error: String },
    
    // Shell Events
    ShellCommandExecuted { command: String, exit_code: i32, output: String },
    ShellProjectSetup { project_type: String, name: String, success: bool },
    
    // Context Events
    ContextAnalysisStarted { path: String },
    ContextAnalysisCompleted { path: String, symbols_found: usize },
    ContextUpdated { file_path: String, change_type: String },
    ...
    ConfigChanged { section: String, key: String, value: serde_json::Value },
    ConfigValidationError { section: String, error: String },
}
...
impl SystemBus {
    ...
    pub async fn register_component(&self, component: SystemComponent) -> (SystemBusHandle, mpsc::UnboundedReceiver<SystemMessage>) {
    ...
    pub async fn subscribe(&self, subscriber_id: String, component: SystemComponent, filter: EventFilter) -> broadcast::Receiver<SystemMessage> {
        {
    ...
    pub async fn publish(&self, event: SystemMessage) -> Result<(), String> {
        ...
        self.event_sender.send(event.clone()).map_err(|e| format!("Failed to broadcast event: {}", e))?;
    ...
    pub async fn get_component_stats(&self) -> HashMap<SystemComponent, ComponentStats> {
        ...
        for (component, sender) in channels.iter() {
            stats.insert(component.clone(), ComponentStats {
    ...
    fn get_target_components(&self, event: &SystemEvent) -> Vec<SystemComponent> {
        match event {
            ...
            SystemEvent::UIUserInput { .. } | SystemEvent::UICommandRequest { .. } => {
                vec![SystemComponent::AgentSystem, SystemComponent::CodeGenerator, SystemComponent::CoordinationEngine]
            }
            
            // Agent events go to UI and coordination
            SystemEvent::AgentTaskStarted { .. } | SystemEvent::AgentTaskCompleted { .. } | SystemEvent::AgentTaskFailed { .. } => {
            ...
            SystemEvent::CodeGenerationCompleted { .. } | SystemEvent::CodeGenerationFailed { .. } => {
                vec![SystemComponent::UI, SystemComponent::AgentSystem]
            }
            
            // AI events go to UI and whoever requested
            SystemEvent::AIRequestStarted { .. } | SystemEvent::AIResponseReceived { .. } | SystemEvent::AIError { .. } => {
                vec![SystemComponent::UI, SystemComponent::CodeGenerator, SystemComponent::AgentSystem]
            }
            
            // Context events go to agents and UI
            SystemEvent::ContextAnalysisCompleted { .. } | SystemEvent::ContextUpdated { .. } => {
                vec![SystemComponent::UI, SystemComponent::AgentSystem, SystemComponent::CodeGenerator]
            }
...
impl SystemBusHandle {
    ...
    pub async fn publish(&self, event: SystemEvent) -> Result<(), String> {
        ...
        self.event_sender.send(message).map_err(|e| format!("Failed to publish event: {}", e))?;
        Ok(())
    }
    
    /// Publish an event with custom metadata
    pub async fn publish_with_metadata(&self, event: SystemEvent, metadata: HashMap<String, serde_json::Value>) -> Result<(), String> {
        let message = SystemMessage {
            id: Uuid::new_v4().to_string(),
            timestamp: chrono::Utc::now(),
            source: format!("{:?}", self.component),
...
impl SystemMessage {
    ...
    pub fn matches_filter(&self, filter: &EventFilter) -> bool {
        ...
        if let Some(event_types) = &filter.event_types {
            let event_type = format!("{:?}", self.event).split('(').next().unwrap_or("").to_string();
