...
use super::client::{
    AnthropicClient, AnthropicConfig, OllamaClient, OllamaConfig, OpenAIClient, OpenAIConfig,
};
...
impl AIManager {
    ...
    pub async fn list_models(&self, provider: &AIProvider) -> Result<Vec<ModelInfo>, AIError> {
        let client = self.clients.get(provider).ok_or_else(|| {
            AIError::ConfigurationError(format!("Provider {:?} not configured", provider))
        })?;
    ...
    pub async fn chat_completion_default(
        &self,
        request: ChatRequest,
    ...
    pub async fn refresh_models(&mut self) -> Result<(), AIError> {
        ...
        for (provider, client) in &self.clients {
            match client.list_models().await {
                ...
                Err(e) => {
                    eprintln!(
                        "Warning: Failed to refresh models from {:?}: {}",
                        provider, e
                    );
                }
            }
    ...
    pub async fn update_config(&mut self, new_config: AIModelConfig) -> Result<(), AIError> {
        ...
        if ollama_changed {
            ...
            let ollama_client = OllamaClient::new(ollama_config);
            self.clients
                .insert(AIProvider::Ollama, Box::new(ollama_client));
        }
    ...
    pub async fn generate_response(
        ...
        parameters.max_tokens = max_tokens
            .map(|t| t as usize)
            .or(Some(self.config.max_tokens));
