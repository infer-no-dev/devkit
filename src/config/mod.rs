...
impl ConfigManager {
    ...
    pub fn with_environment(config_path: Option<PathBuf>, environment: &str) -> Result<Self, ConfigError> {
                ...
                for entry in entries.flatten() {
                    ...
                    if let Some(name) = path.file_name().and_then(|n| n.to_str()) {
                        if name.starts_with("config.") && name.ends_with(".toml") && name != "config.toml" {
                            if let Some(env_name) = name.strip_prefix("config.").and_then(|n| n.strip_suffix(".toml")) {
    ...
    pub fn create_environment(&self, environment: &str, base_on_current: bool) -> Result<(), ConfigError> {
        let env_config_path = self.get_environment_config_path_for(environment);
        
        if env_config_path.exists() {
            return Err(ConfigError::ValidationError(
                format!("Environment '{}' already exists", environment)
            ));
        }
        ...
        self.loader.save_to_file(&config_to_save, &env_config_path)?;
        Ok(())
    }
    ...
    pub fn restore_from_backup(&mut self) -> Result<(), ConfigError> {
        if let Some(backup) = self.config_cache.take() {
            ...
            Err(ConfigError::ValidationError("No backup available".to_string()))
        }
    }
    ...
    fn get_environment_config_path_for(&self, environment: &str) -> PathBuf {
        if environment == "default" {
            ...
            let parent = self.config_path.parent().unwrap_or_else(|| std::path::Path::new("."));
    ...
    fn merge_configs(&mut self, override_config: Config) -> Result<(), ConfigError> {
        ...
        let base_json = serde_json::to_value(&self.config)
            .map_err(|e| ConfigError::MergeError(format!("Failed to serialize base config: {}", e)))?;
        
        let override_json = serde_json::to_value(&override_config)
            .map_err(|e| ConfigError::MergeError(format!("Failed to serialize override config: {}", e)))?;
        
        let merged = self.merge_json_values(base_json, override_json)?;
        
        self.config = serde_json::from_value(merged)
            .map_err(|e| ConfigError::MergeError(format!("Failed to deserialize merged config: {}", e)))?;
        
        Ok(())
    }
    
    fn merge_json_values(&self, base: serde_json::Value, override_val: serde_json::Value) -> Result<serde_json::Value, ConfigError> {
    ...
    fn set_nested_value(&self, json: &mut serde_json::Value, path: &str, value: serde_json::Value) -> Result<(), ConfigError> {
        let parts: Vec<&str> = path.split('.').collect();
        if parts.is_empty() {
        ...
        for (i, part) in parts.iter().enumerate() {
            if i == parts.len() - 1 {
                ...
                match current {
                    ...
                    _ => return Err(ConfigError::ValidationError("Cannot set value on non-object".to_string())),
                }
            } else {
                // Navigate deeper
                match current {
                    ...
                    _ => return Err(ConfigError::ValidationError("Cannot navigate through non-object".to_string())),
                }
            }
        }
