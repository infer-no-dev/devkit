...
use std::fs;
use walkdir::WalkDir;
use crate::context::{FileContext, Dependency, AnalysisConfig, ContextError, DependencyType, DependencySource};
...
impl CodebaseAnalyzer {
    ...
    pub async fn analyze_specific_files(
        ...
        for file_path in file_paths {
            if let Some(root_path) = file_path.parent() {
                match self.analyze_single_file(file_path, &root_path.to_path_buf()).await {
                    Ok(file_context) => file_contexts.push(file_context),
                    Err(e) => {
                        tracing::warn!("Failed to analyze file {:?}: {}", file_path, e);
                        continue;
    ...
    async fn analyze_single_file(
        ...
        let metadata = fs::metadata(file_path)
            .map_err(|e| ContextError::AnalysisFailed(format!("Failed to read metadata for {:?}: {}", file_path, e)))?;
        
        // Try to read as bytes first to check if it's valid UTF-8
        let bytes = fs::read(file_path)
            .map_err(|e| ContextError::AnalysisFailed(format!("Failed to read file {:?}: {}", file_path, e)))?;
        ...
        let content = match String::from_utf8(bytes) {
            ...
            Err(_) => {
                ...
                return Err(ContextError::AnalysisFailed("File contains non-UTF-8 content".to_string()));
            }
        };
    ...
    fn is_build_or_binary_file(&self, file_path: &Path) -> bool {
        ...
        if path_str.contains("/target/") || path_str.contains("\\target\\") ||
           ...
           path_str.contains("/out/") || path_str.contains("\\out\\") ||
        ...
        if let Some(extension) = file_path.extension() {
            ...
            if let Some(filename) = file_path.file_name() {
                ...
                if name.starts_with("lib") && !name.contains(".") {
                    ...
                    !matches!(name.as_str(), "makefile" | "dockerfile" | "vagrantfile" | "readme" | "license" | "changelog")
                }
            } else {
                false
            }
    ...
    fn extract_symbols(&self, content: &str, language: &str) -> Vec<crate::context::symbols::Symbol> {
        ...
        for (line_num, line) in content.lines().enumerate() {
            ...
            if let Some(start) = trimmed.find("fn ") {
                if let Some(name_start) = trimmed[start + 3..].find(char::is_alphabetic) {
                    ...
                    if let Some(name_end) = name_part.find(|c: char| c == '(' || c.is_whitespace()) {
                        ...
                        let mut symbol = Symbol::new(name, SymbolType::Function, PathBuf::new(), line_num + 1, 0);
            ...
            if let Some(start) = trimmed.find("struct ") {
                if let Some(name_start) = trimmed[start + 7..].find(char::is_alphabetic) {
                    ...
                    if let Some(name_end) = name_part.find(|c: char| c.is_whitespace() || c == '{' || c == '<') {
                        let name = name_part[..name_end].to_string();
                        let mut symbol = Symbol::new(name, SymbolType::Struct, PathBuf::new(), line_num + 1, 0);
                        symbol.visibility = if trimmed.starts_with("pub ") {
                            Visibility::Public
                        } else {
                            Visibility::Private
            ...
            if let Some(start) = trimmed.find("enum ") {
                if let Some(name_start) = trimmed[start + 5..].find(char::is_alphabetic) {
                    ...
                    if let Some(name_end) = name_part.find(|c: char| c.is_whitespace() || c == '{' || c == '<') {
                        let name = name_part[..name_end].to_string();
                        symbols.push(Symbol::new(name, SymbolType::Enum, PathBuf::new(), line_num + 1, 0));
                    }
                }
            }
            ...
            if let Some(start) = trimmed.find("trait ") {
                if let Some(name_start) = trimmed[start + 6..].find(char::is_alphabetic) {
                    ...
                    if let Some(name_end) = name_part.find(|c: char| c.is_whitespace() || c == '{' || c == '<') {
                        let name = name_part[..name_end].to_string();
                        symbols.push(Symbol::new(name, SymbolType::Trait, PathBuf::new(), line_num + 1, 0));
                    }
                }
            }
        }
        ...
        for (line_num, line) in content.lines().enumerate() {
            ...
            if trimmed.starts_with("def ") {
                if let Some(name_start) = trimmed[4..].find(char::is_alphabetic) {
                    ...
                    if let Some(name_end) = name_part.find('(') {
                        let name = name_part[..name_end].to_string();
                        let mut symbol = Symbol::new(name.clone(), SymbolType::Function, PathBuf::new(), line_num + 1, 0);
            ...
            if trimmed.starts_with("class ") {
                if let Some(name_start) = trimmed[6..].find(char::is_alphabetic) {
                    ...
                    if let Some(name_end) = name_part.find(|c: char| c.is_whitespace() || c == '(' || c == ':') {
                        let name = name_part[..name_end].to_string();
                        symbols.push(Symbol::new(name, SymbolType::Class, PathBuf::new(), line_num + 1, 0));
                    }
                }
            }
        }
        ...
        for (line_num, line) in content.lines().enumerate() {
            ...
            if trimmed.starts_with("function ") {
                if let Some(name_start) = trimmed[9..].find(char::is_alphabetic) {
                    ...
                    if let Some(name_end) = name_part.find('(') {
                        let name = name_part[..name_end].to_string();
                        symbols.push(Symbol::new(name, SymbolType::Function, PathBuf::new(), line_num + 1, 0));
                    }
                }
            }
            ...
            if trimmed.contains(" => ") {
                if let Some(arrow_pos) = trimmed.find(" => ") {
                    ...
                    if let Some(equals_pos) = before_arrow.rfind('=') {
                        if let Some(_name_start) = before_arrow[..equals_pos].rfind(|c: char| c.is_alphabetic() || c == '_') {
                            let name_part = &before_arrow[..equals_pos];
                            if let Some(actual_name_start) = name_part.rfind(|c: char| c.is_whitespace() || c == '{' || c == ',' || c == '(') {
            ...
            if trimmed.starts_with("interface ") {
                if let Some(name_start) = trimmed[10..].find(char::is_alphabetic) {
                    ...
                    if let Some(name_end) = name_part.find(|c: char| c.is_whitespace() || c == '{' || c == '<') {
                        let name = name_part[..name_end].to_string();
                        symbols.push(Symbol::new(name, SymbolType::Interface, PathBuf::new(), line_num + 1, 0));
                    }
                }
            }
        }
            ...
            if (trimmed.contains("function") || trimmed.contains("def ") || trimmed.contains("fn ")) 
                ...
                for word in trimmed.split_whitespace() {
                    if word.chars().all(|c| c.is_alphanumeric() || c == '_') 
                        ...
                        symbols.push(Symbol::new(word.to_string(), SymbolType::Function, PathBuf::new(), line_num + 1, 0));
                        break;
                    }
                }
            }
