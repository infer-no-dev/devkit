...
impl CodeAnalyzer {
    ...
    pub fn analyze_prompt(
        &self,
        prompt: &str,
    ) -> Result<PromptIntent, crate::codegen::CodeGenError> {
        ...
        match language {
            "rust" => {
                ...
                if !code.contains("#[derive(") && code.contains("struct") {
                    suggestions
                        .push("Consider adding common derives like Debug, Clone".to_string());
                }
            }
            "python" => {
                ...
                if code.contains("except:") {
                    suggestions
                        .push("Use specific exception types instead of bare except".to_string());
                }
            }
                ...
                "rust" => {
                    if line.contains("println!(") && !line.contains("debug") {
                        modifications.push(CodeModification {
                            ...
                            reason: "Use debug logging instead of println for production code"
                                .to_string(),
                            confidence: 0.8,
                        });
                    }
