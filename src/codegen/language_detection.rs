...
impl LanguageDetector {
    ...
    pub fn detect_from_content(&self, content: &str) -> Result<String, CodeGenError> {
        ...
        if let Some((best_language, _)) = language_scores.iter().max_by_key(|(_, score)| *score) {
    ...
    pub fn detect_from_context(&self, context: &CodebaseContext) -> Result<String, CodeGenError> {
        ...
        if let Some((most_common_language, _)) =
            language_counts.iter().max_by_key(|(_, count)| *count)
        {
            Ok(most_common_language.clone())
        } else {
            // Fallback to analyzing metadata
            if let Some((language, _)) = context
                .metadata
                .languages
                .iter()
    ...
    pub fn detect_comprehensive(
        ...
        if let Some((best_language, confidence)) = candidates
            .iter()
