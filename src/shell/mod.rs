...
impl ShellManager {
    ...
    pub async fn git_status(&self) -> Result<CommandResult, ShellError> {
        self.execute_operation(CommandOperation::GitStatus, None).await
    }
    ...
    pub async fn create_directory_structure(&self, paths: &[&str]) -> Result<Vec<CommandResult>, ShellError> {
        let mut results = Vec::new();
    ...
    pub async fn write_file_content(&self, path: &str, content: &str) -> Result<CommandResult, ShellError> {
    ...
    pub async fn read_file_content(&self, path: &str) -> Result<String, ShellError> {
        ...
        if result.exit_code == 0 {
            ...
            Err(ShellError::ExecutionFailed(format!("Failed to read file '{}': {}", path, result.stderr)))
        }
    }
    
    /// Setup a new project with basic structure
    pub async fn setup_project(&self, project_type: &str, name: &str) -> Result<Vec<CommandResult>, ShellError> {
        let mut results = Vec::new();
        ...
        match project_type {
            ...
            _ => {
                return Err(ShellError::UnsupportedShell(format!("Unknown project type: {}", project_type)));
            }
        }
    ...
    pub fn get_command_syntax(&self, operation: CommandOperation) -> String {
        match (&self.current_shell, operation) {
            ...
            // Git operations
            (_, CommandOperation::GitClone { url, directory }) => {
                match directory {
            ...
            (_, CommandOperation::GitBranch { name }) => {
                match name {
            ...
            (_, CommandOperation::CargoTest { package }) => {
                match package {
...
pub enum CommandOperation {
    ...
    GitClone { url: String, directory: Option<String> },
    GitCommit { message: String },
    GitPush,
    GitPull,
    GitStatus,
    GitBranch { name: Option<String> },
    // Build system operations
    CargoBuild { release: bool },
    CargoTest { package: Option<String> },
    CargoRun { args: Vec<String> },
    CargoCheck,
    NpmInstall,
    NpmRun { script: String },
    NpmTest,
    // File operations
