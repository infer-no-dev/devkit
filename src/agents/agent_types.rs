...
use super::{Agent, AgentError, AgentStatus, AgentMetrics, BaseAgent};
use super::task::{AgentTask, AgentResult, AgentArtifact};
...
impl CodeGenerationAgent {
    ...
    async fn generate_code(&mut self, task: &AgentTask) -> Result<AgentResult, AgentError> {
        ...
        let language = task.context
            .get("language")
            .and_then(|l| l.as_str())
            .unwrap_or("rust");
            
        let requirements = task.context
            .get("requirements")
            .and_then(|r| r.as_array())
            .map(|arr| arr.iter().filter_map(|v| v.as_str()).collect::<Vec<_>>())
            .unwrap_or_default();
        
        let file_path = task.context
            .get("file_path")
            .and_then(|p| p.as_str());
        
        let existing_code = task.context
            .get("existing_code")
            .and_then(|c| c.as_str());
        
        // Generate code using AI if available
        let generated_code = if let Some(ai_manager) = &self.ai_manager {
            self.generate_with_ai(ai_manager, &task.description, language, &requirements, existing_code).await?
        } else {
        ...
        .with_metadata("generation_time_ms".to_string(), json!(start_time.elapsed().as_millis()));
...
impl Agent for CodeGenerationAgent {
    ...
    async fn process_task(&mut self, task: AgentTask) -> Result<AgentResult, AgentError> {
        ...
        let result = match task.task_type.as_str() {
            "generate_function" | "generate_class" | "generate_module" | "generate_tests" | "complete_code" | "refactor_code" => {
                self.generate_code(&task).await
            }
            _ => {
                Err(AgentError::InvalidTaskType { task_type: task.task_type.clone() })
            }
        };
...
impl AnalysisAgent {
    ...
    async fn analyze_code(&mut self, task: &AgentTask) -> Result<AgentResult, AgentError> {
        ...
        let code = task.context
            .get("code")
            .and_then(|c| c.as_str())
            .or_else(|| task.context.get("file_path").and_then(|p| p.as_str()))
            .ok_or_else(|| AgentError::TaskExecutionFailed("No code or file path provided".to_string()))?;
        
        let analysis_result = if let Some(ai_manager) = &self.ai_manager {
            self.analyze_with_ai(ai_manager, code, &task.description).await?
        } else {
...
impl RefactoringAgent {
    ...
    async fn refactor_code(&mut self, task: &AgentTask) -> Result<AgentResult, AgentError> {
        ...
        let code = task.context
            .get("existing_code")
            .and_then(|c| c.as_str())
            .ok_or_else(|| AgentError::TaskExecutionFailed("No existing code provided".to_string()))?;
        
        let refactored_code = if let Some(ai_manager) = &self.ai_manager {
            self.refactor_with_ai(ai_manager, code, &task.description).await?
        } else {
        ...
        .with_metadata("refactoring_type".to_string(), json!(task.task_type.clone()));
