...
use agentic_dev_env::{
    config::{Config, ConfigManager, ConfigError},
    testing::{
        TestRunner, TestContext, TestSuiteConfig, TestAssertions,
        TestEnvironment, MockDataFactory, PropertyGenerator,
    },
};
...
"#;
    ...
    let config = config_manager.load_from_path(&config_path).map_err(|e| e.to_string())?;
...
async fn test_config_validation(ctx: TestContext) -> Result<(), String> {
    ...
    if validation_result.is_err() {
        return Err(format!("Valid configuration failed validation: {:?}", validation_result));
    }
    ...
    if invalid_validation_result.is_ok() {
        return Err("Invalid configuration passed validation when it should have failed".to_string());
    }
...
"#;
    ...
    config_manager.switch_environment("development").map_err(|e| e.to_string())?;
    let dev_loaded_config = config_manager.get_config().ok_or("Failed to get dev config")?;
    ...
    config_manager.switch_environment("production").map_err(|e| e.to_string())?;
    let prod_loaded_config = config_manager.get_config().ok_or("Failed to get prod config")?;
...
"#;
    ...
    let initial_loaded = config_manager.get_config().ok_or("Failed to get initial config")?;
...
"#;
    ...
    let hot_reload_detected = config_manager.check_and_reload().map_err(|e| e.to_string())?;
    if !hot_reload_detected {
        return Err("Hot reload did not detect configuration changes".to_string());
    }
    
    let reloaded_config = config_manager.get_config().ok_or("Failed to get reloaded config")?;
...
async fn test_config_backup_restore(ctx: TestContext) -> Result<(), String> {
    ...
    let modified_config = config_manager.get_config().ok_or("Failed to get modified config")?;
    ...
    if restore_result.is_err() {
        return Err(format!("Failed to restore from backup: {:?}", restore_result));
    }
    
    // Verify restoration
    let restored_config = config_manager.get_config().ok_or("Failed to get restored config")?;
    assert_eq!(restored_config.general.log_level, original_config.general.log_level);
    assert_eq!(restored_config.agents.max_concurrent_agents, original_config.agents.max_concurrent_agents);
...
async fn test_config_path_operations(ctx: TestContext) -> Result<(), String> {
    ...
    config_manager.set_value("general.log_level", serde_json::Value::String("error".to_string()))
        .map_err(|e| e.to_string())?;
    
    config_manager.set_value("agents.max_concurrent_agents", serde_json::Value::Number(serde_json::Number::from(15)))
        .map_err(|e| e.to_string())?;
...
async fn test_property_based_config_validation(ctx: TestContext) -> Result<(), String> {
    ...
    for i in 0..5 {
        ...
        let validation_result = config_manager.validate();
        if validation_result.is_err() {
            return Err(format!("Generated configuration {} failed validation: {:?}", i, validation_result));
        }
    }
...
async fn test_config_performance(ctx: TestContext) -> Result<(), String> {
    ...
    for i in 0..100 {
        large_config.agents.custom_agents.push(format!("custom_agent_{}", i));
    }
    
    // Add many environment variables
    for i in 0..50 {
        large_config.shell.environment_variables.insert(
            format!("ENV_VAR_{}", i),
            format!("value_{}", i),
        );
    }
    ...
    for i in 0..200 {
        large_config.keybindings.insert(
            format!("keybinding_{}", i),
            format!("action_{}", i),
        );
    }
    ...
    if validation_result.is_err() {
        return Err(format!("Large configuration validation failed: {:?}", validation_result));
    }
    ...
    ctx.add_metadata("validation_time_ms", &validation_time.as_millis().to_string());
    ctx.add_metadata("serialization_time_ms", &serialization_time.as_millis().to_string());
...
async fn run_config_unit_tests() {
    ...
    let tests = vec![
        ...
        ("property_based_config_validation", test_property_based_config_validation),
...
async fn test_config_edge_cases() {
    ...
    assert!(result.success, "Configuration edge case test failed: {:?}", result.error_message);
}
