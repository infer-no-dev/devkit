...
mod analyzer_tests {
    ...
    fn test_file_analysis_failure() {
        ...
        assert!(matches!(result.unwrap_err(), ContextError::AnalysisFailed(_)));
...
mod symbol_tests {
    ...
    fn test_symbol_search() {
        ...
        let symbols = vec![
            ...
            ContextFixtures::create_symbol("function_two", SymbolType::Function, "src/utils.rs", 30),
    ...
    fn test_symbol_update() {
        ...
        let original_symbol = ContextFixtures::create_symbol(
            "test_func", 
            SymbolType::Function, 
            "src/main.rs", 
            10
...
mod relationship_tests {
    ...
    fn test_code_relationship_creation() {
        ...
        assert!(matches!(relationship.relationship_type, RelationshipType::Uses));
    ...
    fn test_relationship_analysis() {
        ...
        let uses_relationships: Vec<_> = context.relationships.iter()
...
mod repository_tests {
    ...
    async fn test_repository_file_tracking() {
        ...
        assert!(files.iter().any(|f| f.file_name().unwrap_or_default() == "Cargo.toml"));
    ...
    async fn test_repository_status() {
        ...
        let repo_manager = RepositoryManager::new();
        let status = repo_manager.get_repository_status(env.workspace_path()).await;
...
mod integration_tests {
    ...
    fn test_symbol_cross_referencing() {
        ...
        let struct_symbol = ContextFixtures::create_symbol(
            "DataProcessor", 
            SymbolType::Struct, 
            "src/types.rs", 
            10
        ...
        assert_eq!(function_refs[0].location.file, PathBuf::from("src/utils.rs"));
    ...
    fn test_context_incremental_updates() {
        ...
        assert_eq!(search_results[0].location.file, PathBuf::from("src/new_module.rs"));
    }
    ...
    async fn test_context_with_repository_info() {
        ...
        let mut context = ContextFixtures::create_codebase_context(
            env.workspace_path().to_str().unwrap()
        );
        
        // Add repository information to context
        let repo_data = repo_info.unwrap();
        context.metadata.insert("repository_type".to_string(), format!("{:?}", repo_data.repo_type));
        context.metadata.insert("repository_root".to_string(), repo_data.root_path.to_string_lossy().to_string());
        
        if let Some(branch) = repo_data.branch {
            context.metadata.insert("current_branch".to_string(), branch);
        }
    ...
    fn test_context_serialization() {
        ...
        assert_eq!(restored_context.dependencies.len(), context.dependencies.len());
    }
    ...
    fn test_large_codebase_handling() {
        ...
        let file_symbols = context.symbols.search_by_file(&PathBuf::from("src/module_25.rs"));
