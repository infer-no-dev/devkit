...
use agentic_dev_env::testing::{TestEnvironment, TestChannels, TestTime, mocks::MockAgent, fixtures::AgentFixtures};
...
mod agent_manager_tests {
    ...
    async fn test_register_duplicate_agent() {
        ...
        manager.register_agent("test_agent", Box::new(agent1)).await.unwrap();
        let result = manager.register_agent("test_agent", Box::new(agent2)).await;
        
        assert!(result.is_err());
        assert!(matches!(result.unwrap_err(), AgentError::AgentAlreadyExists(_)));
    }
    ...
    async fn test_submit_task() {
        let mut manager = AgentManager::new(3);
        let agent = MockAgent::new("test_agent", "mock");
        manager.register_agent("test_agent", Box::new(agent)).await.unwrap();
    ...
    async fn test_concurrent_task_limit() {
        ...
        // Register 3 agents
        for i in 1..=3 {
            let agent = MockAgent::new(&format!("agent_{}", i), "mock").with_delay(Duration::from_millis(100));
            manager.register_agent(&format!("agent_{}", i), Box::new(agent)).await.unwrap();
        }
    ...
    async fn test_task_completion_callback() {
        ...
        manager.set_completion_callback(Box::new(move |result| {
            completed_tasks_clone.lock().unwrap().push(result.task_id.clone());
        }));
...
mod coordination_tests {
    ...
    async fn test_workflow_with_dependencies() {
        ...
        workflow.add_task(WorkflowTask::from_agent_task(task2, vec!["task_1".to_string()]));
        workflow.add_task(WorkflowTask::from_agent_task(task3, vec!["task_1".to_string(), "task_2".to_string()]));
        
        assert_eq!(workflow.tasks.len(), 3);
        assert!(workflow.tasks[1].dependencies.contains(&"task_1".to_string()));
        assert!(workflow.tasks[2].dependencies.contains(&"task_1".to_string()));
        assert!(workflow.tasks[2].dependencies.contains(&"task_2".to_string()));
    }
    ...
    async fn test_workflow_with_task_failure() {
        ...
        manager.register_agent("failing_agent", Box::new(failing_agent)).await.unwrap();
        
        let coordinator = WorkflowCoordinator::new();
        let mut workflow = Workflow::new("failing_workflow", "Workflow with failing task");
        
        let task = AgentFixtures::create_task("failing_task", "This task will fail", TaskPriority::Normal);
        workflow.add_task(WorkflowTask::from_agent_task(task, Vec::new()));
    ...
    async fn test_debugging_agent() {
        ...
        let task = AgentTask {
            ...
            context: {
                ...
                context.insert("error_message".to_string(), "expected `;` after expression".to_string());
...
mod priority_scheduling_tests {
    ...
    async fn test_task_priority_ordering() {
        ...
        manager.register_agent("priority_agent", Box::new(agent)).await.unwrap();
        ...
        let low_task = AgentFixtures::create_task("low_task", "Low priority task", TaskPriority::Low);
        let high_task = AgentFixtures::create_task("high_task", "High priority task", TaskPriority::High);
        let normal_task = AgentFixtures::create_task("normal_task", "Normal priority task", TaskPriority::Normal);
        let critical_task = AgentFixtures::create_task("critical_task", "Critical priority task", TaskPriority::Critical);
        
        manager.submit_task("priority_agent", low_task).await.unwrap();
        manager.submit_task("priority_agent", high_task).await.unwrap();
        manager.submit_task("priority_agent", normal_task).await.unwrap();
        manager.submit_task("priority_agent", critical_task).await.unwrap();
    ...
    async fn test_task_timeout_handling() {
        ...
        let slow_agent = MockAgent::new("slow_agent", "mock")
            .with_delay(Duration::from_millis(200)); // Longer than timeout
        manager.register_agent("slow_agent", Box::new(slow_agent)).await.unwrap();
        
        // Set a short timeout
        manager.set_task_timeout(Duration::from_millis(100));
        
        let task = AgentFixtures::create_task("timeout_task", "This will timeout", TaskPriority::Normal);
...
mod error_handling_tests {
    ...
    async fn test_agent_error_recovery() {
        ...
        let task = AgentFixtures::create_task("failing_task", "This will fail", TaskPriority::Normal);
        ...
        let recovery_task = AgentFixtures::create_task("recovery_task", "Recovery task", TaskPriority::Normal);
    ...
    async fn test_agent_cancellation() {
        let mut agent = MockAgent::new("cancel_agent", "mock")
            .with_delay(Duration::from_millis(100));
        
        let task = AgentFixtures::create_task("cancel_task", "Task to be cancelled", TaskPriority::Normal);
...
mod integration_tests {
    ...
    async fn test_complete_code_generation_workflow() {
        ...
        manager.register_agent("analyzer", Box::new(analyzer)).await.unwrap();
        manager.register_agent("codegen", Box::new(codegen)).await.unwrap();
        ...
        let codegen_task = AgentFixtures::create_code_generation_task("Add error handling to main function");
        workflow.add_task(WorkflowTask::from_agent_task(codegen_task, vec!["analysis_task".to_string()]));
    ...
    async fn test_multi_agent_coordination() {
        ...
        let tasks = vec![
            ("agent_1", AgentFixtures::create_task("task_1", "Task for agent 1", TaskPriority::Normal)),
            ("agent_2", AgentFixtures::create_task("task_2", "Task for agent 2", TaskPriority::High)),
            ("agent_3", AgentFixtures::create_task("task_3", "Task for agent 3", TaskPriority::Low)),
        ];
    ...
    async fn test_agent_communication_integration() {
        ...
        manager.register_agent("communicator_1", Box::new(agent1)).await.unwrap();
        manager.register_agent("communicator_2", Box::new(agent2)).await.unwrap();
        ...
        assert!(matches!(msg.message_type, MessageType::CollaborationRequest));
    }
}
