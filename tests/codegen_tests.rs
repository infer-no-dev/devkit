...
mod generator_tests {
    ...
    fn test_code_generation_with_context() {
        ...
        context
            .metadata
            .insert("framework".to_string(), "tokio".to_string());
        context
            .metadata
            .insert("language_version".to_string(), "rust-2021".to_string());
    ...
    fn test_code_generation_failure() {
        ...
        assert!(matches!(
            result.unwrap_err(),
            CodegenError::GenerationFailed(_)
        ));
...
mod template_tests {
    ...
    fn test_complex_template_rendering() {
        ...
        variables.insert(
            ...
            "Represents a person with basic information".to_string(),
        );
        variables.insert(
            "fields".to_string(),
            "name: String,\n    age: u32,".to_string(),
...
mod analyzer_tests {
    ...
    fn test_code_improvement_suggestions() {
        ...
        assert!(suggestions
            .iter()
            .any(|s| s.category == "code_duplication" || s.category == "magic_numbers"));
    }
}
...
mod integration_tests {
    ...
    fn test_template_based_generation() {
        ...
        variables.insert(
            "params".to_string(),
            "username: &str, password: &str".to_string(),
        );
        variables.insert(
            "return_type".to_string(),
            "Result<bool, AuthError>".to_string(),
        );
        variables.insert(
            "body".to_string(),
    ...
    fn test_context_aware_generation() {
        ...
        context
            .metadata
            .insert("async_runtime".to_string(), "tokio".to_string());
        context
            .metadata
            .insert("database".to_string(), "postgresql".to_string());
        context
            .metadata
            .insert("web_framework".to_string(), "axum".to_string());
    ...
    fn test_generation_with_validation() {
        ...
        assert!(validation_messages
            .iter()
            .any(|msg| msg.contains("validation passed")));
    }
    ...
    fn test_incremental_code_generation() {
        ...
        let methods_result =
            generator.generate_code("Add validation methods to user struct", &context, &config);
        assert!(methods_result.is_ok());

        // Generate tests
        let tests_result =
            generator.generate_code("Create unit tests for user struct", &context, &config);
