...
use devkit::blueprint::languages::{MultiLanguageAnalyzer, analyzers::create_analyzer_for_language};
...
"#).await?;
        ...
        match lang_analyzer.analyze(temp_path).await {
            Ok(module) => {
                ...
                for (i, dep) in module.dependencies.iter().take(3).enumerate() {
                    println!("   Dep {}: {} v{} ({})", i+1, dep.name, dep.version, dep.purpose);
                }
    ...
    println!("   Secondary languages: {:?}", blueprint.secondary_languages);
    println!("   Language modules: {}", blueprint.language_modules.len());
    
    for (lang, module) in &blueprint.language_modules {
        println!("   - {:?}: {} dependencies, {} entry points", 
                lang, module.dependencies.len(), module.entry_points.len());
    }
    
    // Test interface extraction
    println!("\n🔌 Testing Cross-Language Interface Detection...");
    println!("✅ Detected {} cross-language interfaces", blueprint.inter_language_interfaces.len());
    ...
    println!("✅ Build orchestration: {}", blueprint.build_orchestration.orchestration_tool);
    println!("   - Build order: {:?}", blueprint.build_orchestration.build_order);
    println!("   - Parallel builds: {}", blueprint.build_orchestration.parallel_builds);
    
    // Test deployment strategy
    println!("\n🚀 Testing Deployment Strategy...");
    println!("✅ Deployment platform: {}", blueprint.deployment_strategy.orchestration.platform);
