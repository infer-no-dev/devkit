...
use std::env;
use devkit::shell::{ShellManager, CommandOperation, ShellConfig};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let echo_result = shell_manager.execute_command("echo 'Hello from shell!'", None).await?;
    ...
    println!("Create directory result: exit code {}", mkdir_result.exit_code);
    
    // Write to a test file
    let test_content = "Hello from Shell Manager!\\nThis is a test file.";
    let test_file = format!("{}/test.txt", test_dir);
    
    let write_result = shell_manager.write_file_content(&test_file, test_content).await?;
    ...
    let ls_result = shell_manager.execute_operation(CommandOperation::ListFiles, None).await?;
    ...
    for cmd in &commands_to_check {
        ...
        let status = if exists { "✅ Available" } else { "❌ Not found" };
        println!("  {}: {}", cmd, status);
    }
        ...
        match shell_manager.git_status().await {
            Ok(status_result) => {
                ...
                if status_result.exit_code == 0 {
                    println!("  Status: {}", status_result.stdout.lines().take(3).collect::<Vec<_>>().join("\\n"));
                } else {
    ...
    if shell_manager.command_exists("cargo").await {
        ...
        if check_result.exit_code == 0 {
            ...
            println!("  Error output: {}", check_result.stderr.lines().take(3).collect::<Vec<_>>().join("\\n"));
        }
    }
    ...
    if env::current_dir()?.join("temp_test").exists() == false {
        ...
        match shell_manager.setup_project("rust", temp_project).await {
            Ok(results) => {
                ...
                for (i, result) in results.iter().enumerate() {
                    println!("  Operation {}: exit code {} - execution time {} ms", 
                        i + 1, 
                        result.exit_code,
